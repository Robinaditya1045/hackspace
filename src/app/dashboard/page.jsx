  ...
  return (
    <div className="relative h-screen w-screen bg-black text-white flex flex-col ">
      ...
      <div className="flex justify-between items-center p-6">
        <h1 className="text-4xl border-b border-red-500 font-mono text-white-300">
          Your Workspaces :
        </h1>

        <Button
          onClick={createWorkspace}
          className="relative inline-flex items-center px-6 py-3 bg-gradient-to-r from-red-600 to-yellow-300 text-white font-semibold rounded-lg shadow-md group hover:from-purple-700 hover:to-blue-700 transform transition-all duration-300 ease-out hover:scale-105 z-10"
          disabled={isCreating}
        >
          {isCreating ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {workspaces.length === 0 ? (
              ...
              workspaces.map((ws) => (
                <Card
                  key={ws.id}
                  className="relative group border border-red-900 bg-opacity-10 backdrop-blur-md p-2 bg-black-900 rounded-xl transition-all duration-300 ease-in-out hover:scale-[1.02] hover:shadow-lg hover:shadow-red-500/50"
                >
                  <CardContent className="p-6 flex flex-col gap-4">
                    <Link href={`/workspace/${ws.id}`} className="block">
                      <div className="flex flex-col gap-3">
                        <h2 className="text-3xl font-bold text-white-400 tracking-wide group-hover:text-blue-300 transition-colors">
                          {ws.name}
                        </h2>
          ...
          <DialogDescription>
            ...
            <div className="flex space-x-4">
              <Button
                ...
                className="flex items-center gap-2 px-5 py-2 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-500 text-white font-semibold rounded-lg shadow-lg transition-all duration-300 transform hover:scale-105"
                disabled={isCreating}
              >
                {isCreating ? (
                  <Loader2 className="h-5 w-5 animate-spin" />
